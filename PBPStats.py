    ...
    with tab11:
        ...
        with tab112:
            ...
            interactive_table(
                ...
                columnDefs=[{"className": "dt-center", "targets": "_all"}])
    with tab12:
        tab121, tab122 = st.tabs(['Against', 'Favor'])
        with tab121:

            unf_team = unf['Team'].value_counts().reset_index().reset_index()
            unf_team['Rank'] = unf_team['index'] + 1
            unf_team.drop('index', axis=1, inplace=True)
            interactive_table(
                unf_team.set_index('Rank').rename(columns={'count': 'Unsportsmanlike Foul'}),
                paging=False, height=2000, width=2000, showIndex=True,
                classes="display order-column nowrap table_with_monospace_font", searching=True,
                fixedColumns=True, select=True, info=False, scrollCollapse=True,
                scrollX=True, scrollY=1000, fixedHeader=True, scroller=True, filter='bottom',
        ...
        with tab122:
            unf_team = unf['Against'].value_counts().reset_index().reset_index()
            unf_team['Rank'] = unf_team['index'] + 1
            unf_team.drop('index', axis=1, inplace=True)
            interactive_table(
                unf_team.set_index('Rank').rename(columns={'count': 'Unsportsmanlike Foul'}),
...
with tab2:
    ...
    with tab21:
        ...
        interactive_table(
            ...
            columnDefs=[{"className": "dt-center", "targets": "_all"}])
    with tab22:
        tab211, tab212 = st.tabs(['Against', 'Favor'])
        with tab211:
            coach_timeouts_pergame = coachtimeout[['Coach', 'Fixture']].value_counts().reset_index().groupby('Coach')[
                'count'].mean().reset_index().round(1).sort_values("count", ascending=False).reset_index().reset_index()

            coach_timeouts_pergame['Rank'] = coach_timeouts_pergame['level_0'] + 1
            coach_timeouts_pergame.drop(['index', 'level_0'], axis=1, inplace=True)
            interactive_table(
                coach_timeouts_pergame.set_index('Rank').rename(columns={'count': 'Time outs/Game'}),
                paging=False, height=2000, width=2000, showIndex=True,
                classes="display order-column nowrap table_with_monospace_font", searching=True,
                fixedColumns=True, select=True, info=False, scrollCollapse=True,
                scrollX=True, scrollY=1000, fixedHeader=True, scroller=True, filter='bottom',
                columnDefs=[{"className": "dt-center", "targets": "_all"}])
    with tab212:
        against_coach_timeouts_pergame =coachtimeout[['Against Coach', 'Fixture']].value_counts().reset_index().groupby('Against Coach')[
            'count'].mean().reset_index().round(1).sort_values("count", ascending=False).reset_index().reset_index()

        against_coach_timeouts_pergame['Rank'] = against_coach_timeouts_pergame['level_0'] + 1
        against_coach_timeouts_pergame.drop(['index', 'level_0'], axis=1, inplace=True)
        interactive_table(
            against_coach_timeouts_pergame.set_index('Rank').rename(columns={'count': 'Time outs/Game'}),
            paging=False, height=2000, width=2000, showIndex=True,
            classes="display order-column nowrap table_with_monospace_font", searching=True,
            fixedColumns=True, select=True, info=False, scrollCollapse=True,
            scrollX=True, scrollY=1000, fixedHeader=True, scroller=True, filter='bottom',
...
with tab3:
    tab31, tab32,tab33 = st.tabs(['Technical Foul', 'Unsportsmanlike Foul','4th Period Points'])
    with tab31:
        player_technicals_fouls = tfouls['Player'].loc[tfouls.PLAYTYPE == 'CMT'].value_counts().reset_index().reset_index()
    ...
    with tab32:
        ...
        unf_player = unf['Player'].value_counts().reset_index().reset_index()
        unf_player['Rank'] = unf_player['index'] + 1
        unf_player.drop('index', axis=1, inplace=True)
        interactive_table(
            unf_player.set_index('Rank').rename(columns={'count': 'Unsportsmanlike Foul'}),
            paging=False, height=2000, width=2000, showIndex=True,
            classes="display order-column nowrap table_with_monospace_font", searching=True,
            fixedColumns=True, select=True, info=False, scrollCollapse=True,
            scrollX=True, scrollY=1000, fixedHeader=True, scroller=True, filter='bottom',
            columnDefs=[{"className": "dt-center", "targets": "_all"}])
    with tab33:
        points_compute_total = pbp_data_df2425.loc[pbp_data_df2425.PLAYTYPE.isin(['2FGM', '3FGM', 'FTM'])][
            ['Team', 'PLAYTYPE', 'Player', 'PERIOD']].value_counts().reset_index()
        points_compute_total['PTS'] = points_compute_total['PLAYTYPE'].apply(points_scored) * points_compute_total[
            'count']
        player_points_by_period = points_compute_total.groupby(['Player', 'PERIOD'])['PTS'].sum().reset_index()
        tab331,tab332,tab333,tab334=st.tabs(['4th Period points',"Last 5 minutes points", "Last 2 minutes points",'Last minute points'])
        with tab331:
            player_points_by_period4 = \
            player_points_by_period.loc[player_points_by_period.PERIOD == 4].sort_values('PTS', ascending=False)[
                ['Player', 'PTS']]

            interactive_table(
                player_points_by_period4.head(20),
                ...
                columnDefs=[{"className": "dt-center", "targets": "_all"}])
        with tab332:
            points_compute_total_last_5_min = pbp_data_2425.loc[
                (pbp_data_2425.PERIOD == 4) & (pbp_data_2425.MINUTE > 35) & (
                    pbp_data_2425.PLAYTYPE.isin(['2FGM', '3FGM', 'FTM']))][
                ['Team', 'PLAYTYPE', 'Player']].value_counts().reset_index()

            points_compute_total_last_5_min['PTS'] = points_compute_total_last_5_min['PLAYTYPE'].apply(points_scored) * \
                                                     points_compute_total_last_5_min['count']
            points_compute_total_last_5_min = points_compute_total_last_5_min.groupby(['Player'])[
                'PTS'].sum().reset_index().sort_values('PTS', ascending=False).head(15)

            interactive_table(
                points_compute_total_last_5_min.head(20),
                paging=False, height=900, width=2000, showIndex=False,
                classes="display order-column nowrap table_with_monospace_font", searching=False,
                fixedColumns=True, select=True, info=False, scrollCollapse=True,
                scrollX=True, scrollY=1000, fixedHeader=True, scroller=True, filter='bottom',
        ...
        with tab333:
            points_compute_total_last_5_min = pbp_data_2425.loc[
                (pbp_data_2425.PERIOD == 4) & (pbp_data_2425.MINUTE > 38) & (
                    pbp_data_2425.PLAYTYPE.isin(['2FGM', '3FGM', 'FTM']))][
                ['Team', 'PLAYTYPE', 'Player']].value_counts().reset_index()
        ...
        with tab334:
            points_compute_total_last_5_min = pbp_data_2425.loc[
                (pbp_data_2425.PERIOD == 4) & (pbp_data_2425.MINUTE > 39) & (
                    pbp_data_2425.PLAYTYPE.isin(['2FGM', '3FGM', 'FTM']))][
                ['Team', 'PLAYTYPE', 'Player']].value_counts().reset_index()
